...
class SubjectData(object):
    ...
    **kwargs: param-value dict_like
    ...
    def _delete_orientation(self):
        ...
        for attr in ['session_output_dirs']:
    ...
    def _sanitize_output_dirs(self):
        for output_dir in [self.output_dir, self.scratch]:
            self._sanitize_output_dir(output_dir)
        ...
        if self.anat is not None:
            ...
            self.anat_scratch_dir = (
                self.anat_output_dir if self.scratch is None
                else self.scratch)

        if self.func is not None:
            if self.session_output_dirs is None:
                self.session_output_dirs = [
                    os.path.join(self.output_dir, sess_id)
                    for sess_id in self.session_ids]
            self.session_scratch_dirs = (
                self.session_output_dirs if self.scratch is None
                else [os.path.join(self.scratch, sess_id)
                      for sess_id in self.session_ids])

            for output_dir in self.session_output_dirs:
                self._sanitize_output_dir(output_dir)
            for output_dir in self.session_scratch_dirs:
                self._sanitize_output_dir(output_dir)
    ...
    def _niigz2nii(self):
        ...
        if None not in [self.func, self.session_scratch_dirs]:
    ...
    def _set_session_ids(self):
        ...
        if self.session_ids is None:
            ...
            if isinstance(self.session_ids, (_basestring, int)):
                ...
                assert len(self.session_ids) == self.n_sessions, "%s != %s" % (
                    len(self.session_ids), self.n_sessions)
    ...
    def generate_realignment_thumbnails(self, log=True, nipype=True):
        ...
        self._set_session_ids()
        if not hasattr(self, 'realignment_parameters'):
            warnings.warn("'realignment_parameters' attribute not set!")
            return
        if not self.reporting_enabled():
            self.init_report()
